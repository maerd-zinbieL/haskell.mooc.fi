...
ex6_myHead_full = forAll_ $ \(NonEmpty xs :: NonEmptyList Int) ->
  counterexample ("xs = " ++ show xs)
...
ex7_myLast_full = forAll_ $ \(NonEmpty xs :: NonEmptyList Int) ->
  counterexample ("xs = " ++ show xs)
