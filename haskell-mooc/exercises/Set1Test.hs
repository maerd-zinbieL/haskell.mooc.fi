...
tests =
  [ ( 1
    , "onetwo"
    , [ex1_one_defined, ex1_one_type, ex1_two_defined, ex1_two_type]
    )
  , (2, "double"       , [ex2_double])
...
ex4_distance = do
  (deltax, deltay, dist) <- elements
    ([(3, 4, 5), (5, 12, 13), (8, 15, 17), (7, 24, 25)] :: [ ( Double
        , Double
        , Double
        )
      ]
...
ex7_corners = $(testing [|postagePrice 500|]) (?== 250)
  .&&. $(testing [|postagePrice 5000|]) (?== 5300)

ex7_cheap =
  forAllBlind (choose (0, 500)) $ \w -> $(testing [|postagePrice w|]) (?== 250)

ex7_medium = forAllBlind (choose (501, 5000))
  $ \w -> $(testing [|postagePrice w|]) (?== 300 + w)

ex7_large = forAllBlind (choose (5001, 10000))
  $ \w -> $(testing [|postagePrice w|]) (?== 6000)
...
ex9_sumTo = forAllBlind (elements [1 .. 100])
  $ \n -> $(testing [|sumTo n|]) (?== sum [1 .. n])

ex10_power = forAllBlind (elements [1 .. 10]) $ \n ->
  forAllBlind (elements [1 .. 10]) $ \k -> $(testing [|power n k|]) (?== n ^ k)
