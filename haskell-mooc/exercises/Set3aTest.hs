...
tests =
  ...
  , ( 2
    , "mapMaybe"
    , [ex2_mapMaybe_Nothing, ex2_mapMaybe_Just_String, ex2_mapMaybe_Just_Int]
    )
  ...
  , ( 12
    , "multiCompose"
    , [ex12_empty, ex12_scalability, ex12_arithmetic, ex12_strings]
    )
  , ( 13
    , "multiApp"
    , [ex13_empty, ex13_scalability, ex13_arithmetic, ex13_strings, ex13_mixed]
    )
...
ex5_capitalize_1 = property $ do
  ...
  w1  <- word
  w2  <- word
  return $ $(testing [|capitalize (cap:w1++" "++cap:w2)|])
    (?== (toUpper cap : w1 ++ " " ++ toUpper cap : w2))
...
ex5_capitalize_2 = property $ do
  ...
  return . $(testing [|capitalize input|]) $ \v -> conjoin
    [ counterexample ("char at index " ++ show p) $ v !! p ?== toUpper
      (input !! p)
    , not (elem q positions)
      ==> counterexample ("char at index " ++ show q) (v !! q ?== input !! q)
    ]
...
m_powers maxlen = property $ do
 ...
 where
  check n 0 = True
  check n 1 = True
  check n k | k `mod` n == 0 = check n (div k n)
            | otherwise      = False
...
ex8_whileRight_Left = forAllBlind word $ \w ->
  forAllBlind (choose (1 :: Int, 10)) $ \i ->
    counterexample
        ("let f _ = Left " ++ show w ++ " in whileRight f " ++ show i)
      $ within timeLimit
...
ex12_empty = property $ do
  ...
  let input  = multiCompose [] n
      output = n
  return $ counterexample ("multiCompose [] " ++ show n) $ input ?== output
...
ex12_strings = property $ do
  ...
  return
    $   counterexample
          ("multiCompose [(++\"xy\"), reverse, tail, reverse] " ++ show s)
    $   input
    ?== output
...
ex13_empty = property $ do
  ...
  let input  = multiApp id [] n
      output = [] :: [Int]
  return $ counterexample ("multiApp id [] " ++ show n) $ input ?== output

ex13_scalability = property $ do
  n <- choose (0, 9) :: Gen Int
  k <- choose (0, 9) :: Gen Int
  let input =
        multiApp (sum :: [Int] -> Int) (replicate n (succ :: Int -> Int)) k
      output = n * (k + 1)
  return
    $   counterexample
          ("multiApp sum (replicate " ++ show n ++ " succ) " ++ show k)
    $   input
    ?== output
...
ex13_strings = property $ do
  ...
  return
    $   counterexample
          ("multiApp id [(\\xs -> [head xs]), tail, reverse]" ++ show words)
    $   input
    ?== output
...
ex14_interpreter_1 = property $ do
  ...
  let first =
        replicate up "up" ++ replicate right "right" ++ ["printY", "printX"]
      second =
        replicate down "down" ++ replicate left "left" ++ ["printY", "printX"]
      input  = first ++ second
