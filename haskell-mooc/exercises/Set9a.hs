...
workload nExercises hoursPerExercise | total > 100 = "Holy moly!"
...
countValid []       = 0
countValid (s : ss) = valid s + countValid ss
 where
  valid :: String -> Int
  valid s | length s < 6     = 0
          | s !! 2 == s !! 4 = 1
          | s !! 3 == s !! 5 = 1
          | otherwise        = 0
...
sumSuccess xs = go xs 0 0
 where
  go []             _   0     = Left "no data"
  go []             sum count = Right sum
  go (Right i : xs) sum count = go xs (i + sum) (1 + count)
  go (_       : xs) sum count = go xs sum count
...
open code' c@(ClosedLock code) | code' == code = OpenLock code
                              --  | otherwise     = c
open _ clock = clock
...
instance Eq Text where
  (Text []) == (Text []) = True
  (Text []) == (Text ys) = all isSpace ys
  (Text xs) == (Text []) = all isSpace xs
  (Text (x : xs)) == (Text (y : ys)) | isSpace x = Text xs == Text (y : ys)
                                     | isSpace y = Text (x : xs) == Text ys
                                     | otherwise = x == y && Text xs == Text ys
...
compose []              _    = []
compose ((k, v) : map1) map2 = case lookup v map2 of
  Nothing   -> compose map1 map2
  (Just v') -> (k, v') : compose map1 map2
...
permute is xs = permuteHelper (map (`lookup` kvs) (identity (length kvs)))
  where kvs = zip is xs

permuteHelper :: [Maybe a] -> [a]
permuteHelper []            = []
permuteHelper (Just x : xs) = x : permuteHelper xs
permuteHelper (_      : xs) = permuteHelper xs
