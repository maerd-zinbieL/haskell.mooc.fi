...
ex1_ok = forAllBlind word $ \for -> forAllBlind word $ \sur ->
  let str = for ++ " " ++ sur
  in  $(testing [|readNames str|]) (?== Just (for, sur))

m_ex1_fail s = $(testing [|readNames s|]) (?== Nothing)

ex1_fail = forAllBlind word $ \for -> forAllBlind word $ \sur ->
  forAllBlind bad $ \b -> conjoin
    [ m_ex1_fail (for ++ sur)
...
ex2_small = forAllBlind word $ \p1 ->
  forAllBlind (word `suchThat` (/= p1)) $ \p2 ->
    forAllBlind (choose (1, 20)) $ \s1 -> forAllBlind (choose (0, s1)) $ \s2 ->
      forAllBlind (shuffle [(p1, s1), (p2, s2)]) $ \inp -> conjoin
        [ $(testing [|winner inp p1 p2|]) (?== Just p1)
        , s2 < s1 ==> $(testing [|winner inp p2 p1|]) (?== Just p1)
        ]

ex2_big = forAllBlind word $ \w -> forAllBlind (choose (10, 100)) $ \s ->
  forAllBlind (nub <$> listOf1 (word `suchThat` (/= w))) $ \names ->
    forAllBlind (vectorOf (length names) (choose (0, s - 1))) $ \scores ->
      forAllBlind (elements names) $ \other ->
        forAllBlind (shuffle ((w, s) : zip names scores)) $ \inp -> conjoin
          [ $(testing [|winner inp other w|]) (?== Just w)
          , $(testing [|winner inp w other|]) (?== Just w)
          ]

ex2_fail = forAllBlind (nub <$> listOf1 word) $ \names ->
...
ex3_ok = forAll_ $ \(as :: [Int]) ->
  forAllBlind (shuffle (zip [0 ..] as)) $ \pairs ->
    forAllShrink_ (sublistOf pairs) $ \selected ->
      $(testing [|selectSum as (map fst selected)|])
        (?== Just (sum $ map snd selected))
...
ex4 = property $ do
  ...
  return
    ...
    $   countAndLog (== i) input
    ?== Logger (replicate n (show i)) n

ex5_balance_examples = conjoin
  [ $(testing' [|runBankOp (balance "harry") exampleBank|])
    (?== (10, exampleBank))
  , $(testing' [|runBankOp (balance "sean") exampleBank|])
    (?== (0, exampleBank))
  ]
...
ex5_balance_ok = forAllBlind bank $ \b ->
  forAllBlind (elements (Map.keys b)) $ \account ->
    let bank = Bank b
    in
      $(testing' [|runBankOp (balance account) bank|]) $ \out ->
        counterexample
            (  " with bank = "
            ++ show bank
            ++ "\n      account = "
...
ex6_rob = property $ do
  ...
  let run from to bank exp =
        $(testing' [|runBankOp (rob from to) bank|]) $ \out ->
          counterexample
              (  " with bank = "
              ++ show bank
              ++ "\n      from = "
...
ex8_paren = forAll_ $ \(NonNegative i) -> conjoin
...
ex8_parensMatch = forAllBlind genMatch $ \match -> conjoin
  [ $(testing [|parensMatch match|]) (?== True)
  , forAllBlind (breakMatch match)
    $ \broken -> $(testing [|parensMatch broken|]) (?== False)
  , forAllBlind (filterMatch match)
    $ \broken -> $(testing [|parensMatch broken|]) (?== False)
  ]
...
ex9 = property $ do
  ...
  let Just cx = lookup x assocs
      s x =
        counterexample ("runState (count " ++ show x ++ ") " ++ show assocs)
          . counterexample " Final state"
  return $ conjoin
    [ s y $ hasElements ((y, 1) : assocs) (execState (count y) assocs)
    , s x $ hasElements ((x, cx + 1) : delete (x, cx) assocs)
                        (execState (count x) assocs)
    ]

ex10_1 = forAllShrink_ (nub <$> listOf (choose ('a', 'z'))) $ \cs ->
