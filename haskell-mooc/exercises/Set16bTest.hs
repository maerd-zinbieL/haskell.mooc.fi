...
ex2_v = forAllBlind niceFloat $ \r1 -> forAllBlind niceFloat $ \r2 ->
...
ex2_t1 = counterexample "eurToChf = composeRates eurToUsd usdToChf"
...
ex4_v2 = forAllBlind word $ \f -> forAllBlind word $ \l ->
  counterexample
      ("fromName (toFull (toFirst " ++ show f ++ ") (toLast " ++ show l ++ "))")
    $   fromName (toFull (toFirst f) (toLast l))
    ?== f
    ++  " "
    ++  l
