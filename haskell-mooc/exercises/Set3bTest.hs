...
tests =
  ...
  , ( 8
    , "mymaximum"
    , [imports, ex8_mymaximum_max, ex8_mymaximum_min, ex8_mymaximum_empty]
    )
...
ex1_buildList = property $ do
  ...
  return $ $(testing [|buildList start count end|])
    (?== (replicate count start ++ [end]))
...
ex3_mylast_empty =
  forAll_ $ \(b :: Bool) -> $(testing [|mylast b ([]::[Bool])|]) (?== b)
...
ex5_sorted_empty = $(testing [|sorted ([]::[Int])|]) (?== True)
...
ex8_mymaximum_max = forAll_ $ \(NonEmpty xs :: NonEmptyList Int) ->
  counterexample ("mymaximum (>) 0 " ++ show xs) $ mymaximum (>) 0 xs ?== max
    0
    (maximum xs)
...
ex10_maybeMap_1 = forAll_ $ \bs ->
  counterexample
      ("let f True = Just True; f False = Nothing in maybeMap f " ++ show bs)
    $   maybeMap f bs
    ?== filter id bs
 where
  f True  = Just True
