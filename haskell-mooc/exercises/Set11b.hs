...
appendAll r ss = modifyIORef r (\s -> s ++ concat ss)
...
-- swapIORefs r1@(newIORef c1) r2@(newIORef c2) = do writeIORef r1 c2
--                                                  writeIORef r2 c1
swapIORefs r1 r2 = do
  c1 <- readIORef r1
  c2 <- readIORef r2
...
hFetchLines h = go []
 where
  go lines = do
    isEnd <- hIsEOF h
    if isEnd
      then return $ reverse lines -- lazy?
      else do
        line <- hGetLine h
        go $ line : lines
...
hSelectLines h nums = go [] 1 $ sort nums
 where
  go lines _ [] = return $ reverse lines
  go lines i (l : ls)
    | i == l = do
      line <- hGetLine h
      go (line : lines) (i + 1) ls
    | otherwise = do
      line <- hGetLine h
      go lines (i + 1) (l : ls)
...
interact' f state = do
  ...
  next $ f (input, state)
 where
  next (True, s, state') = do
    putStrLn s
    interact' f state'
  next (False, s, state') = do
    putStrLn s
    return state'
