...
member e (Set []) = False
member e (Set (x : xs)) | e == x    = True
                        | otherwise = member e (Set xs)

-- add a member to a set
add :: Ord a => a -> Set a -> Set a
add e (Set []) = Set [e]
add e (Set (x : xs)) | e < x     = Set (e : x : xs)
                     | e == x    = Set (x : xs)
                     | otherwise = Set (x : xs')
  where Set xs' = add e (Set xs)
...
data State = Start | Error | Finished | Eggs | EggsFlour | EggsSugar | EggsSugarFlour | Mixed
  deriving (Eq,Show)
...
step EggsFlour      AddSugar = EggsSugarFlour
step EggsSugar      AddFlour = EggsSugarFlour
step EggsSugarFlour Mix      = Mixed
...
instance Ord a => Semigroup (Set a) where
  Set [] <> setY = setY
  Set (x : xs) <> setY | member x setY = Set xs <> setY
                       | otherwise     = Set xs <> add x setY
...
passwordAllowed pass (ContainsSome  str) = or [ c `elem` str | c <- pass ]
passwordAllowed pass (DoesNotContain str) =
  not (passwordAllowed pass (ContainsSome str))
passwordAllowed pass (And r1 r2) =
  passwordAllowed pass r1 && passwordAllowed pass r2
passwordAllowed pass (Or r1 r2) =
  passwordAllowed pass r1 || passwordAllowed pass r2
...
data Arithmetic = Value Integer | Add Arithmetic Arithmetic | Mul Arithmetic Arithmetic
  deriving Show
...
evaluate (Mul i j) = evaluate i * evaluate j


render :: Arithmetic -> String
render (Value i) = show i
render (Add i j) = "(" ++ render i ++ "+" ++ render j ++ ")"
render (Mul i j) = "(" ++ render i ++ "*" ++ render j ++ ")"
