...
outputInInput input output = forAll (elements output) 
                                    (\pair-> snd pair === occurs (fst pair) input)
                              where occurs key xs = foldr (\x occur -> if x == key then occur+1 else occur) 0 xs
...
frequenciesProp freq input = wrap sumIsLength input .&&.  
                             wrap inputInOutput input .&&.
                             wrap outputInInput input 
  where wrap propTest (NonEmpty input') = propTest input' (freq input')
...
genList = do
  len <- choose (3,5)
  let gen = elements [0..10]
  vector <- vectorOf len gen
  return (sort vector)
...
instance Arbitrary Arg where
  arbitrary = do
    let varGen = elements (map Variable ['a','b','c','x','y','z'])
        numGen = elements (map Number [0..10])
    oneof [varGen, numGen]
...
instance Arbitrary Expression where
  arbitrary = do
    ...
    let minusGen = return $ Minus arg1 arg2
        plusGen = return $ Plus arg1 arg2
    oneof [minusGen, plusGen]
