...
fmap2 f cc = fmap (fmap f) cc

fmap3
  :: (Functor f, Functor g, Functor h) => (a -> b) -> f (g (h a)) -> f (g (h b))
fmap3 f ccc = fmap (fmap2 f) ccc
...
count y xs = foldr (\x folded -> if x == y then 1 + folded else folded) 0 xs
...
inBoth xs ys = foldr help [] xs
 where
  help x folded | x `elem` ys = x : folded
                | otherwise   = folded
...
instance Foldable List where
  ...
  foldr f init (LNode x rest) = f x (foldr f init rest)
...
instance Foldable TwoList where
  ...
  foldr f init (TwoNode l r rest) = f l (f r (foldr f init rest))
...
fmap_ :: (a -> b) -> Fun a -> Fun b
fmap_ f (Fun int2a) = Fun (f . int2a)
