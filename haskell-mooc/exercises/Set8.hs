...
render (Picture f) w h name = writePng
  name
  (generateImage (\x y -> colorToPixel (f (Coord x y))) w h)
 where
...
blendColor (Color r1 g1 b1) (Color r2 g2 b2) = Color (ave r1 r2)
                                                     (ave g1 g2)
                                                     (ave b1 b2)
  where ave x y = div (x + y) 2

combine :: (Color -> Color -> Color) -> Picture -> Picture -> Picture
combine f0 (Picture f1) (Picture f2) = Picture f3
  where f3 coord = f0 (f1 coord) (f2 coord)
...
dot :: Int -> Int -> Shape
dot x y = Shape f where f (Coord cx cy) = (x == cx) && (y == cy)
...
union (Shape f1) (Shape f2) = Shape f3 where f3 coord = f1 coord || f2 coord
...
paintSolid color (Shape s) (Picture g) = Picture g'
 where
...
examplePatterns =
  ( paint (solid black)        hat
    . paint (stripes red yellow) legs
    . paint (stipple pink black) body
    )
    allWhite
 where
  legs = circle 50 200 250
...
xy :: Picture
xy = Picture f where f (Coord x y) = Color (mod x 256) (mod y 256) 0
...
instance Transform Fill where
  apply (Fill color) _ = Picture g where g coord = color

data Zoom = Zoom Int
  deriving Show

instance Transform Zoom where
  apply (Zoom z) (Picture base) = Picture (base . zoomCoord z)

data Flip = FlipX | FlipY | FlipXY
  deriving Show

instance Transform Flip where
  apply FlipX (Picture base) = Picture p
    where p (Coord x y) = base (Coord (-x) y)
  apply FlipY (Picture base) = Picture p
    where p (Coord x y) = base (Coord x (-y))
  apply FlipXY (Picture base) = Picture p
    where p (Coord x y) = base (Coord y x)
------------------------------------------------------------------------------
...
instance (Transform a, Transform b)=> Transform (Chain a b) where
  apply (Chain t2 t1) pic = apply t2 (apply t1 pic)
------------------------------------------------------------------------------
...
blurColor (Color r1 g1 b1) (Color r2 g2 b2) (Color r3 g3 b3) (Color r4 g4 b4) (Color r5 g5 b5)
  = Color (div (r1 + r2 + r3 + r4 + r5) 5)
          (div (g1 + g2 + g3 + g4 + g5) 5)
          (div (b1 + b2 + b3 + b4 + b5) 5)

instance Transform Blur where
  apply blur (Picture base) = Picture p
   where
    p (Coord x y) = blurColor (base (Coord x y))
                              (base (Coord (x + 1) y))
                              (base (Coord (x - 1) y))
                              (base (Coord x (y + 1)))
                              (base (Coord x (y - 1)))
