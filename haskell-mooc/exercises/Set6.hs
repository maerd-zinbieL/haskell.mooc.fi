...
simplify (RationalNumber p q) =
  RationalNumber (p `div` gcd p q) (q `div` gcd p q)
...
instance Num RationalNumber where
  RationalNumber p1 q1 + RationalNumber p2 q2 = simplify r
    where r = RationalNumber (p1 * q2 + p2 * q1) (q1 * q2)
  RationalNumber p1 q1 * RationalNumber p2 q2 = simplify r
    where r = RationalNumber (p1 * p2) (q1 * q2)
  abs (RationalNumber p q) = simplify (RationalNumber (abs p) q)
  signum (RationalNumber p q) = RationalNumber (signum p) 1
  fromInteger x = RationalNumber x 1
  negate (RationalNumber p q) = simplify (RationalNumber (-p) q)
