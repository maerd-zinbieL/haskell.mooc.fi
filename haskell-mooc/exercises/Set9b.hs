...
prettyPrint n queens = go queens (initBoard n)
 where
  go :: [Coord] -> [[Char]] -> String
  go []                    board = concat board
  go ((row, col) : queens) board = go queens (updateBoard 'Q' (row, col) board)

initBoard :: Int -> [[Char]]
initBoard n = replicate n (replicate n '.' ++ ['\n'])

updateBoard :: Char -> Coord -> [[Char]] -> [[Char]]
updateBoard c (row, col) board =
  take (row - 1) board ++ updateRow col (board !! (row - 1)) : drop row board
  where updateRow col cs = take (col - 1) cs ++ c : drop col cs
--------------------------------------------------------------------------------
...
danger cand [] = False
danger cand (q : queens) | sameRow cand q      = True
                         | sameCol cand q      = True
                         | sameDiag cand q     = True
                         | sameAntidiag cand q = True
                         | otherwise           = danger cand queens
...
prettyPrint2 n qs = concat (queenPrint qs (dangerPrint (1, 1) (initBoard n)))
 where
  queenPrint :: [Coord] -> [[Char]] -> [[Char]]
  queenPrint []            b = b
  queenPrint ((r, c) : qs) b = queenPrint qs (updateBoard 'Q' (r, c) b)
  dangerPrint :: Coord -> [[Char]] -> [[Char]]
  dangerPrint (r, c) b
    | r == n + 1       = b
    | c == n + 1       = dangerPrint (r + 1, 1) b
    | danger (r, c) qs = dangerPrint (r, c + 1) (updateBoard '#' (r, c) b)
    | otherwise        = dangerPrint (r, c + 1) b
--------------------------------------------------------------------------------
...
fixFirst n ((r, c) : qs) = go c qs
 where
  go :: Int -> Stack -> Maybe Stack
  go c qs | c >= n + 1       = Nothing
          | danger (r, c) qs = go (c + 1) qs
...
finish n qs = go qs
 where
  go :: Stack -> Stack
  go (q : qs) | length (q : qs) == n + 1 = qs
              | otherwise                = go $ step n (q : qs)
