...
import           Set13b                  hiding ( perhapsIncrement
                                                , sumBounded
                                                , sumNotTwice
                                                )
...
ex1_maybe = forAll_ $ \(c :: Char) -> forAll_ $ \(d :: Char) ->
  forAll_ $ \(b :: Bool) ->
    let
      nc = Nothing :: Maybe Char
      nb = Nothing :: Maybe Bool
    in
      conjoin
        [ $(testing [|ifM (Just b) (Just c) (Just d)|])
          (?== Just (if b then c else d))
        , $(testing [|ifM (Just b) (Just c) nc|])
          (?== if b then Just c else Nothing)
        , $(testing [|ifM (Just b) nc (Just d)|])
          (?== if b then Nothing else Just d)
        , $(testing [|ifM nb (Just c) (Just d)|]) (?== Nothing)
        ]

ex1_state = conjoin
  [ $(testing' [|runState (ifM get (return 'a') (return 'b')) True|])
    (?== ('a', True))
  , $(testing' [|runState (ifM get (return 'a') (return 'b')) False|])
    (?== ('b', False))
  ]

ex2_maybe = forAll_ $ \(as :: [Int]) -> forAll_ $ \(bs :: [Int]) ->
  let z   = zipWith (-) as bs
      res = if all (/= 0) z then Just z else Nothing
  in  counterexample
          (  "mapM2 (\\x y -> if x == y then Nothing else Just (x-y)) "
          ++ show as
          ++ " "
          ++ show bs
          )
        $   mapM2 (\x y -> if x == y then Nothing else Just (x - y)) as bs
        ?== res
...
ex2_state = forAll_ $ \(bs :: [Bool]) -> forAll_ $ \(is :: [Int]) ->
  counterexample
      ("runState (mapM2 perhapsIncrement " ++ show bs ++ " " ++ show is ++ ") 0"
      )
    $   runState (mapM2 perhapsIncrement bs is) 0
    ?== ( zipWith (\_ _ -> ()) is bs
        , sum $ zipWith (\b i -> if b then i else 0) bs is
        )

ex3_visit_examples = conjoin
  [ $(testing' [|runState (visit maze1 "Pit") []|]) (?== ((), ["Pit"]))
  , $(testing' [|runState (visit maze1 "Corridor 2") []|])
    (?== ((), ["Corridor 3", "Corridor 2"]))
  , $(testing' [|runState (visit maze1 "Entry") []|])
    (?== ((), ["Dead end", "Corridor 1", "Pit", "Entry"]))
  , $(testing' [|runState (visit maze1 "Entry") ["Corridor 1"]|])
...
ex3_visit_tree = forAllBlind (choose (1 :: Int, 4)) $ \depth ->
  forAllShrinkBlind (genTree depth "0") shrinkTree $ \maze ->
    ...
    in
      counterexample
        ("runState (visit " ++ show maze ++ " " ++ show root ++ ") []")
      $ within timeLimit
      $ ($ runState (visit maze root) [])
      . was
      $ \(_, state) ->
          counterexample " Final state" $ hasElements (map fst maze) state
...
ex3_visit_linear = forAllBlind (choose (2 :: Int, 6)) $ \n ->
  forAllBlind (choose (0, n - 1)) $ \start ->
    forAllBlind (choose (start, n)) $ \end ->
      let
        maze =
          [ (show i, [show (i + 1)]) | i <- [0 .. n - 1] ] ++ [(show n, [])]
        sstart = show start
        send   = show end
      in
        counterexample
        ...
        $ within timeLimit
        $ ($ runState (visit maze sstart) [send])
        . was
        $ \(_, state) -> counterexample " Final state"
...
ex3_visit = property $ do
  ...
  return
    $ counterexample
        ("runState (visit " ++ show maze ++ " " ++ head names1 ++ ") []")
    $ within timeLimit
    $ ($ runState (visit maze (head names1)) [])
    . was
    $ \(_, state) -> counterexample " Final state" $ hasElements names1 state

ex3_path_examples = conjoin
...
ex3_path_linear = forAllBlind (choose (2 :: Int, 6)) $ \n ->
  forAllBlind (choose (0, n)) $ \start -> forAllBlind (choose (0, n)) $ \end ->
    let maze =
          [ (show i, [show (i + 1)]) | i <- [0 .. n - 1] ] ++ [(show n, [])]
        sstart = show start
        send   = show end
...
ex3_path = property $ do
  ...
  from <- elements names
  to   <- elements names
  return $ $(testing [|path maze from to|])
    (?== (elem from names1 == elem to names1))

ex4_simple = conjoin
  [ $(testing [|findSum2 [6,1] [7]|]) (hasElements [(1, 6, 7), (6, 1, 7)])
  , $(testing [|findSum2 [6,1] [8]|]) (?== [])
  , $(testing [|findSum2 [2,1] [4]|]) (hasElements [(2, 2, 4)])
  , $(testing [|findSum2 [1,2,3,4] [6,7]|])
    (hasElements [(2, 4, 6), (3, 3, 6), (3, 4, 7), (4, 2, 6), (4, 3, 7)])
  ]
...
ex4 = forAllShrink_ (choose (1, 10)) $ \n -> (n > 0 ==>) $ property $ do
  ...
  let
    ...
    triples1 =
      [ (vals !! (i1 - k), vals !! (i1 + k), v1) | k <- [-lim1 .. lim1] ]
...
ex5_bits = forAllShrink_ (choose (1 :: Int, 9)) $ \n ->
  ...
  in  conjoin
        [ forAllBlind (elements vals) $ \del ->
          del > 0 ==> $(testing [|allSums (delete del vals)|])
            (hasElementsDuplicates (filter ((== 0) . (Data.Bits..&. del)) all))
...
ex6_sumBounded_ok = forAllShrink_ (listOf1 (choose (-10, 10))) $ \is ->
  forAllBlind (choose (0, 3)) $ \diff ->
    let k = m is + diff in $(testing [|sumBounded k is|]) (?== Just (sum is))

ex6_sumBounded_fail = forAllBlind (listOf1 (choose (-10, 10))) $ \is ->
  forAllBlind (choose (1 :: Int, 3)) $ \diff ->
    let k = m is - diff in $(testing [|sumBounded k is|]) (?== Nothing)

sumNotTwice :: [Int] -> Int
sumNotTwice xs = fst $ runState (foldM f2 0 xs) []

ex6_sumNotTwice = forAllShrink_ (listOf1 (choose (-5, 5)))
  $ \is -> $(testing [|sumNotTwice is|]) (?== sum (nub is))
...
ex8_fmap_2 = property $ do
  m <- word
  s <- choose (0, 10)
  let op = fmap (const True) (msgSL m)
  return
    $   counterexample
          ("runSL (fmap (const True) (msgSL " ++ show m ++ ")) " ++ show s)
    $   runSL op s
    ?== (True, s, [m])
...
ex8_2 = property $ do
  ...
  return
    $   counterexample (s ++ "\n\nrunSL op " ++ show j)
    $   runSL op j
    ?== (j, j + i, [msg, msg2 ++ show j])

ex8_stress = forAll_ $ \ops ->
