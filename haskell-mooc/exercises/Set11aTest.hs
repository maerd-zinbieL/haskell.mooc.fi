...
ex1_hello =
  $(testing [|hello|]) . withNoInput $ checkOutput (?== "HELLO\nWORLD\n")

word = listOf1 (choose ('a', 'z'))

ex2_greet = forAllBlind word $ \name ->
  $(testing [|greet name|]) . withNoInput $ checkOutput
    (?== ("HELLO " ++ name ++ "\n"))
...
ex4_readWords = forAllBlind (listOf1 word) $ \words ->
  $(testing [|readWords (length words - 1)|])
    . withInput (unlines words)
    $ checkResult (?== sort (init words))

ex5_readUntil = forAllBlind word $ \end ->
...
ex6 = forAllBlind (choose (0, 40)) $ \n ->
  $(testing [|countdownPrint n|]) . withNoInput $ checkOutput
    (?== unlines (map show [n, n - 1 .. 0]))

ex7_isums = forAllBlind (listOf1 (choose (-10, 10))) $ \nums ->
  $(testing [|isums (length nums)|])
    . withInput (unlines $ map show nums)
...
ex9_example = forAllBlind (choose (0, 10)) $ \i ->
  $(testing' [|while ask (putStrLn "YAY!")|])
    . withInput (unlines $ replicate i "Y" ++ ["N"])
    $ checkOutput
        (?== unlines (concat (replicate i ["Y/N?", "YAY!"]) ++ ["Y/N?"]))

ex9_False =
  $(testing' [|while (return False) (putStrLn "IMPOSSIBLE")|])
    . withNoInput
    $ checkOutput (?== "")

ex10_debug = forAllBlind word $ \message -> forAllBlind word $ \output ->
  forAllBlind word $ \result ->
    counterexample
      ...
      $ ($ (debug message (putStrLn output >> return result)))
      . withNoInput
