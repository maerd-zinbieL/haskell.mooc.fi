...
ex2 = forAllBlind (listOf word) $ \up0 -> forAllBlind (listOf word) $ \low ->
...
ex3 = forAllBlind (listOf (choose (1, 8))) $ \is ->
  forAllBlind (vectorOf (length is) letter `suchThat` noRepeats) $ \cs ->
    ...
    in  counterexample' ("longestRepeat (T.pack " ++ show inp ++ ")")
          $   within timeLimit
          $   longestRepeat (T.pack inp)
          ?== out

ex4 = forAllBlind word $ \w -> forAllShrink_ (choose (0, 200)) $ \n ->
  (n >= 0 ==>)
    $   counterexample'
          ("takeStrict " ++ show n ++ " (TL.pack (cycle " ++ show w ++ "))")
    $   within timeLimit
    $   takeStrict n (TL.pack (cycle w))
    ?== T.pack (take (fromIntegral n) (cycle w))

ex5 = forAll_ $ \bytes ->
  counterexample' ("byteRange (B.pack " ++ show bytes ++ ")")
    $   byteRange (B.pack bytes)
    ?== if null bytes then 0 else maximum bytes - minimum bytes

ex6_simple = once $ conjoin
  [ counterexample "xorChecksum (B.pack [])" $ xorChecksum (B.pack []) ?== 0
  , counterexample "xorChecksum (B.pack [17])"
  $   xorChecksum (B.pack [17])
  ?== 17
  ]

ex6 = forAll_ $ \w1 -> forAll_ $ \w2 -> forAll_ $ \w3 -> forAll_ $ \chaff ->
  forAllBlind (shuffle ([w1, w2, w3] ++ chaff ++ chaff)) $ \input ->
    counterexample' ("xorChecksum (B.pack " ++ show input ++ ")")
      $   xorChecksum (B.pack input)
      ?== xor w1 (xor w2 w3)
...
ex7_ok = forAllBlind (choose (0, 10)) $ \l ->
  forAllBlind (vectorOf l maybeExoticLetter) $ \s ->
    counterexample' ("countUtf8Chars (encodeUtf8 (T.pack " ++ show s ++ "))")
      $   countUtf8Chars (encodeUtf8 (T.pack s))
      ?== Just l
