...
assertNormalConstructor (Weird n        ) = counterexample
  ("  constructor "
  ++ n
  ++ " is not a normal constructor. Perhaps it's a record constructor or an infix constructor? Make it normal."
  )
  False

assertNoFields (Constructor n vs) =
  counterexample ("  constructor " ++ n ++ " should have no fields") $ null vs
assertNoFields _ = property True

assertFields exp (Constructor n vs) =
  counterexample ("  constructor " ++ n ++ " field types") $ vs ?== exp
assertFields _ _ = property True

ex1 = $(reifyType "Vehicle") $ \(DataType vars cs) -> conjoin
...
ex2 = $(reifyType "BusTicket") $ \(DataType vars cs) -> conjoin
  [ counterexample "  should have no type parameters" $ vars == []
  , counterexample "  should have 2 constructors" $ length cs ?== 2
  , counterexample "  constructor names" $ hasElements
    ["SingleTicket", "MonthlyTicket"]
    [ n | Constructor n _ <- cs ]
  , conjoin $ map assertNormalConstructor cs
  , counterexample "  should have a constructor SingleTicket with no fields"
    $ any (== Constructor "SingleTicket" []) cs
  , counterexample
    "  should have a constructor MonthlyTicket with a String field"
...
ex3_totalPrice = forAllBlind word $ \name -> forAll_ $ \count ->
  forAllBlind (elements [0.1, 0.25 .. 2.0]) $ \price ->
    $(testing [|totalPrice (MkShoppingEntry name price count)|])
      (?~= price * fromIntegral count)

eq (MkShoppingEntry n p c) (MkShoppingEntry n' p' c') =
  and [n == n', p == p', c == c']

ex3_buyOneMore = forAllBlind word $ \name -> forAll_ $ \(price, count) ->
  let exp = MkShoppingEntry name price (succ count)
  in  $(testing [|buyOneMore (MkShoppingEntry name price count)|])
        (\v -> expectation exp v (eq exp v))

ex4_name = property $ do
  n <- word
  return
    $   counterexample ("getName (setName " ++ show n ++ " fred)")
    $   getName (setName n fred)
    === n
...
ex5_generate = forAllShrink_ (choose (0, 20)) $ \a ->
  forAllShrink_ (choose (0, 20)) $ \b' ->
    ...
    in  counterexample
            (  "After starting from origin and going right "
            ++ show a
            ++ " times and up "
            ++ show b
            ++ " times:"
...
ex6_nth = forAllBlind (choose (1, 6))
  $ \i -> $(testing [|study (NthYear i)|]) (?== NthYear (i + 1))
ex6_graduate = $(testing [|study (NthYear 7)|]) (?== Graduated)
ex6_graduated = $(testing [|study Graduated|]) (?== Graduated)

ex7_type = $(reifyType "UpDown") $ \(DataType vars cs) -> conjoin
  [ counterexample "  should have no type parameters" $ vars == []
  , counterexample "  should have 2 constructors" $ length cs ?== 2
...
ex7_generate = forAllShrink_ (choose (0, 20)) $ \a ->
  forAllShrink_ (choose (0, 20)) $ \b ->
    let tc0 = iterate tick zero !! a
        tc1 = iterate tick (toggle tc0) !! b
    in  counterexample
            ("Did " ++ show a ++ " ticks, a toggle, and " ++ show b ++ " ticks:"
            )
...
fcmp expected actual = expectation expected actual (diff < eps)
 where
...
ex8_simple = property $ do
  ...
  return $ conjoin
    ...
    , $(testing [|rgb (Invert (Mix (Mix c1 c2) (Mix c3 c4)))|])
      (fcmp [0.25, 0.75, 1.0])
    ]
...
ex9 = $(reifyType "OneOrTwo") $ \(DataType vars cs) -> conjoin
  ...
  , conjoin $ map assertNormalConstructor cs
  , counterexample
      "  should have a constructor One with one field of the parameter type"
    $ any ((==) $ Constructor "One" $ map SimpleType vars) cs
  , counterexample
      "  should have a constructor Two with two fields of the parameter type"
    $ any ((==) $ Constructor "Two" $ map SimpleType $ vars ++ vars) cs
  ]

ex10_type = $(reifyType "KeyVals") $ \(DataType vars cs) -> conjoin
  ...
  , counterexample "  should have a constructor Pair with the right fields"
    $ any
        (== Constructor
          "Pair"
          [SimpleType "k", SimpleType "v", SimpleType "KeyVals k v"]
        )
        cs
  ]

m_ex10 vs =
  counterexample ("toList (fromList " ++ show vs ++ "))") $ vs ==? toList
    (fromList vs)
...
ex11_just = forAll_ $ \(n :: Nat) -> counterexample
  ("  toNat (fromNat " ++ show n ++ ")")
  (toNat (fromNat n) ?== Just n)
ex11_nothing =
  forAll_ $ \(Negative (z :: Int)) -> $(testing [|toNat z|]) (?== Nothing)
...
ex12_from_zero = counterexample ("fromBin (O End)") (fromBin (O End) ?== 0)
ex12_to_zero = counterexample ("toBin 0") (toBin 0 ?== O End)
ex12_to_from_zero =
  counterexample ("toBin (fromBin (O End))") (toBin (fromBin (O End)) ?== O End)
ex12_from_to_zero =
  counterexample ("fromBin (toBin 0)") (fromBin (toBin 0) ?== 0)
ex12_from_one = counterexample ("fromBin (I End)") (fromBin (I End) ?== 1)
ex12_to_one = counterexample ("toBin 1") (toBin 1 ?== I End)
ex12_to_from_one =
  counterexample ("toBin (fromBin (I End))") (toBin (fromBin (I End)) ?== I End)
ex12_from_to_one =
  counterexample ("fromBin (toBin 1)") (fromBin (toBin 1) ?== 1)
ex12_to_from = forAll_ $ \(b :: Bin) -> counterexample
  ("toBin (fromBin (" ++ show b ++ "))")
  (toBin (fromBin b) ?== b)
ex12_from_to = forAll_ $ \(n :: Int) -> n < 0 .||. counterexample
  ("fromBin (toBin (" ++ show n ++ "))")
  (fromBin (toBin n) ?== n)
