...
cull val (Node val' l r) | val == val' = Empty
...
--     (Just ss, _      ) -> Just (StepL : ss)
--     (Nothing, Just ss) -> Just (StepR : ss)
search val (Node v l r)
  | val == v = Just []
  | otherwise = case search val l of
    Nothing -> case search val r of
      Nothing -> Nothing
      Just ss -> Just (StepR : ss)
    Just ss -> Just (StepL : ss)
