...
sumsHelp :: Int -> Int -> Int -> [Int]
sumsHelp k sum i | k >= i    = [sum + k]
                 | otherwise = (sum + k) : sumsHelp (k + 1) (sum + k) i
------------------------------------------------------------------------------
...
sumsOf []             = []
sumsOf [x           ] = [x]
sumsOf (x0 : x1 : xs) = x0 : sumsOf (x0 + x1 : xs)
...
maybeMap f []       = []
maybeMap f (a : as) = call (f a)
 where
  call (Just b) = b : maybeMap f as
  call Nothing  = maybeMap f as
