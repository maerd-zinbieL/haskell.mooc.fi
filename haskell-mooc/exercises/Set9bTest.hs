...
tests =
  ...
  , ( 2
    , "prettyPrint"
    , [ex2_examples, ex2_size, ex2_content, ex2_comm, ex2_correctness]
    )
  , ( 3
    , "relations"
    , [ ex3_sameRow_pos
  ...
  , (5, "prettyPrint2", [ex5_size, ex5_content, ex5_comm, ex5_correctness])
  , ( 6
    , "fixFirst"
    , [ex6_fixFirst_safeZone, ex6_fixFirst_dangerZone, ex6_fixFirst_outside]
    )
  , (7, "stackOps"    , [ex7_continue, ex7_backtrack])
  , (8, "nqueens_step", [ex8_step_4, ex8_step_continue, ex8_step_backtrack])
  , ( 9
    , "nqueens_finish"
    , [ex9_finish_small, ex9_finish_medium, ex9_finish_large]
    )
  ]
...
ex2_size = property $ do
  ...
  return $ $(testing [| prettyPrint n xs |]) $ \out ->
    let ys = lines out
    in  conjoin
          [ counterexample "  number of lines in output" $ length ys ?== n
...
ex4_danger_neg_2 = property $ do
  ...
  return $ (not (null xs) ==>) $ forAllBlind (elements xs) $ \x ->
...
ex5_correctness = property $ do
  ...
  let check _      []          = True
      check (i, j) ("" : rows) = check (i + 1, 1) rows
      check (i, j) ((char : row) : rows)
        | char == 'Q' = S.member (i, j) q && check (i, j + 1) (row : rows)
        | char == '#' = not (S.member (i, j) q) && S.member (i, j) d && check
          (i, j + 1)
          (row : rows)
...
ex6_fixFirst_dangerZone = property $ do
  ...
  return $ $(testing [| fixFirst n (q:qs) |]) . was $ \x -> case x of
    x@(Just (q'@(qi, qj) : qs')) -> conjoin
      [ counterexample ("  but " ++ show q' ++ " is on the wrong row")
                       (qi ?== i)
      , counterexample ("  but " ++ show q' ++ " is in danger") (q' `elem` sz)
      , counterexample ("  but " ++ show qs' ++ " was not " ++ show qs)
                       (qs' == qs)
      ]
    x@(Just []) -> counterexample ("  but the list was empty") False
    Nothing     -> counterexample
      ("  but there were safe squares on row " ++ show i ++ ": " ++ show
        (intersect row sz)
      )
      (all (`elem` dz) row)
      where row = [ (i, k) | k <- [j .. n] ]
...
ex7_continue = property $ do
  ...
  case continue cs of
    [] ->
      return $ counterexample ("continue " ++ input ++ " returned []") False
    cs' -> return $ conjoin
      [ counterexample ("length (continue " ++ input ++ ")")
                       (length cs' ?== length cs + 1)
      , counterexample ("fst (head (continue " ++ input ++ "))")
      ...
      , counterexample
        ...
        (snd (head cs') >= 1)
      , counterexample ("tail (continue " ++ input ++ ")") (tail cs' ?== cs)
      ]
...
ex7_backtrack = property $ do
  ...
  case backtrack cs of
    [] ->
      return $ counterexample ("backtrack " ++ input ++ " returned []") False
    cs' -> return $ conjoin
      [ counterexample ("length (backtrack " ++ input ++ ")")
                       (length cs' ?== length cs - 1)
      , counterexample ("(head (backtrack " ++ input ++ "))")
                       ((head cs') ?== (fst (cs !! 1), snd (cs !! 1) + 1))
      , counterexample ("tail (backtrack " ++ input ++ ")")
...
ex8_step_4 = conjoin
  [ $(testing [|step 4 from|]) (?== to)
  | (from, to) <- zip steps_4 (tail steps_4)
  ]
...
ex8_step_continue = property $ do
  ...
  return $ not (null s) ==> do
    ...
    return $ $(testing [|step n ((row,col):qs)|]) . was $ \res -> conjoin
...
m_finish_sol sol = forAllBlind (elements (init (tails sol)))
  $ \pre -> $(testing [|finish (length sol) pre|]) (?== sol)

ex9_finish_small = conjoin
...
m_finish_n n = $(testing [|finish n [(1,1)]|]) . was $ \qs -> conjoin
  [ counterexample "  length of list" $ length qs ?== n
  , counterexample "  queens outside board"
  $   []
  ==? [ (i, j) | (i, j) <- qs, i < 1 || i > n || j < 1 || j > n ]
  , let sz = foldr intersect (full n) . map (\q -> q : safeZone n q) $ qs
    in  counterexample "  these queens can capture each other: "
        $   (qs \\ sz)
        ?== []
  ]
