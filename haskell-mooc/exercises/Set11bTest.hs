...
tests =
  ...
  , ( 7
    , "interact'"
    , [ex7_interact_terminates, ex7_interact_example, ex7_interact_loop]
    )
  ]
...
ex1 = forAllBlind word $ \init -> forAllBlind (listOf word) $ \ws ->
  counterexample ("appendAll ref " ++ show ws)
    ...
    $ do
        ...
        stop_ $ counterexample " Final value in ref" $ out ?== concat
          (init : ws)

ex2 = forAllBlind word $ \w1 -> forAllBlind word $ \w2 ->
...
ex3_doubleCall = forAllBlind (choose (0, 20 :: Int)) $ \i ->
  counterexample ("doubleCall (return (return " ++ show i ++ "))")
    $ ($ doubleCall (return (return i)))
    . withNoInput
    $ check (?== "") (?== i)

ex3_doubleCall_2 = forAllBlind word $ \w1 -> forAllBlind word $ \w2 ->
  counterexample
      (  "doubleCall (do putStrLn "
      ++ show w1
      ++ "; return (putStrLn "
      ++ show w2
      ++ "))"
...
ex4_composeIO_1 = forAllBlind word $ \w -> forAllBlind word $ \v ->
...
ex4_composeIO_2 = forAll_ $ \(i :: Int) ->
  counterexample
      ("compose (\\x -> return (x*2)) (\\y -> return (y+1)) " ++ show i)
    $ ($ compose (return . (* 2)) (return . (+ 1)) i)
    . withNoInput
    $ checkResult (?== (i + 1) * 2)
...
ex6_hSelectLines = forAllBlind (listOf1 word) $ \lines ->
  forAllBlind (fmap (nub . sort) . listOf1 $ choose (1, length lines))
    $ \inds ->
        counterexample ("Contents of handle h:\n" ++ unlines lines)
          ...
          $ do
              ...
              length (concat outs) `seq` do
                run $ hClose h
                stop_ $ counterexample ("  was: " ++ show outs) $ conjoin
                  [ counterexample "Length of result"
                  $   length outs
                  ?== length inds
                  , conjoin
                    [ counterexample ("result!!" ++ show i)
                      $   outs
                      !!  i
                      ?== lines
                      !!  ((inds !! i) - 1)
...
ex7_interact_terminates = forAllBlind word $ \w1 -> forAllBlind word $ \w2 ->
  forAllBlind (choose (0 :: Int, 100)) $ \state ->
    counterexample
        (  "let f (input,state) = (False,input,state) in interact' f "
        ++ show state
        )
      $ ($ let f (input, state) = (False, input, state) in interact' f state)
      . withInput (unlines [w1, w2])
      $ check (?== (w1 ++ "\n")) (?== state)

ex7_interact_example =
  $(testing' [|interact' counter 1|])
    . withInput (unlines ["print", "inc", "inc", "print", "quit"])
    $ check (?== unlines ["1", "done", "done", "3", "bye bye"]) (?== 3)

ex7_interact_loop = forAllBlind (choose (0 :: Int, 100)) $ \state ->
  forAllBlind (choose (0 :: Int, 5)) $ \steps -> forAllBlind word $ \w ->
    forAllBlind (word `suchThat` (/= w)) $ \end ->
      let input  = unlines $ replicate steps w ++ [end]
          output = unlines $ map show [state .. state + steps]
      in  counterexample
              (  "let f (input,state) = (input=="
              ++ show w
              ++ ",show state,state+1) in interact' f "
              ++ show state
              )
            $ ($ let f (input, state) = (input == w, show state, state + 1)
                 in  interact' f state
              )
            . withInput input
