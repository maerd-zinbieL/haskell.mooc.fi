...
ex2_small = forAllBlind word $ \name ->
  forAllBlind (choose (0, 10)) $ \amount1 ->
    forAllBlind (choose (0, 10)) $ \amount2 ->
      counterexample "After running"
        $ counterexample ("  db <- openDatabase \"\"")
        $ counterexample
            ("  deposit db (T.pack " ++ show name ++ ") " ++ show amount1)
        $ counterexample
            ("  deposit db (T.pack " ++ show name ++ ") " ++ show amount2)
...
ex2_big = forAllBlind word $ \name ->
  forAllBlind (listOf (choose (0, 10))) $ \amounts ->
    forAllBlind (listOf1 $ (,) <$> suchThat word (/= name) <*> choose (0, 10))
      $ \chaff ->
...
ex3 = forAllBlind word $ \name -> forAllBlind (choose (0, 9999)) $ \amount ->
  conjoin
    [ counterexample
      ("parseCommand [T.pack \"balance\", T.pack " ++ show name ++ "]")
    $   parseCommand [T.pack "balance", T.pack name]
    ?== Just (Balance (T.pack name))
    , counterexample
      (  "parseCommand [T.pack \"deposit\", T.pack "
      ++ show name
      ++ ", T.pack "
      ++ show (show amount)
      ++ "]"
...
ex4_small = forAllBlind word $ \name ->
  forAllBlind (choose (0, 10)) $ \amount ->
    counterexample "Running"
      $ counterexample "  db <- openDatabase \"\""
      $ counterexample
          (  "  out1 <- perform db (Just (Deposit (T.pack "
          ++ show name
          ++ ") "
          ++ show amount
          ++ "))"
      ...
      $ counterexample
          ("  out2 <- perform db (Just (Balance (T.pack " ++ show name ++ ")))")
...
ex4_big = forAllBlind word $ \name ->
  forAllBlind (listOf (choose (0, 10))) $ \amounts ->
    forAllBlind
        (listOf1 $ mkDeposit <$> suchThat word (/= name) <*> choose (0, 10))
      $ \chaff ->
          let input0 = map (mkDeposit name) amounts ++ chaff
          in
            forAllBlind (shuffle input0) $ \input1 ->
              let input = input1 ++ [Just (Balance (T.pack name))]
              in
                counterexample
                  "Note: T.pack calls left out due to technical reasons."
                $ counterexample "Running"
                $ counterexample "  db <- openDatabase \"\""
                ...
                $ do
                    ...
                    stop_
                      $   out
                      ?== (replicate (length input - 1) (T.pack "OK"))
                      ++  [T.pack (show (sum amounts))]
...
ex6_small =
  once $ testHTTP ["/deposit/lopez/17", "/balance/lopez"] ["OK", "17"]
...
ex6_big = forAllBlind word $ \name ->
  forAllBlind (listOf (choose (0, 10))) $ \amounts ->
    forAllBlind
        (listOf1 $ depositPath <$> suchThat word (/= name) <*> choose (0, 10))
      $ \chaff ->
          ...
          in
            forAllBlind (shuffle input0) $ \input1 ->
              ...
              in  testHTTP
                    input
                    ((replicate (length input - 1) "OK") ++ [show (sum amounts)]
                    )

ex7_small = once $ testHTTP
  ["/deposit/simon/17", "/withdraw/simon/6", "/balance/simon"]
  ["OK", "OK", "11"]

ex7_big = forAllBlind word $ \name ->
...
ex8_http = forAllBlind word $ \w -> forAllBlind word $ \name ->
  forAllBlind (choose (0, 10 :: Int)) $ \amount -> conjoin
    [ isError ("/" ++ w)
    , isError ("/deposit/" ++ w)
    , isError ("/withdraw/" ++ w)
