...
withdraw db name amount = execute db depositQuery (name, -amount)
------------------------------------------------------------------------------
...
balance db name = do
  amounts <- query db balanceQuery [name] :: IO [[Int]]
...
parseWithdraw name amount = do
  value <- parseInt amount
  return (Withdraw name value)

parseDeposit :: T.Text -> T.Text -> Maybe Command
parseDeposit name amount = do
  value <- parseInt amount
  return (Deposit name value)

parseBalance :: T.Text -> Maybe Command
parseBalance name = Just (Balance name)

parseCommand :: [T.Text] -> Maybe Command
parseCommand [command, name] | command == T.pack "balance" = parseBalance name
                             | otherwise                   = Nothing
parseCommand [command, name, amount]
  | command == T.pack "deposit"  = parseDeposit name amount
  | command == T.pack "withdraw" = parseWithdraw name amount
  | otherwise                    = Nothing
