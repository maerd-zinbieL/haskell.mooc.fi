...
getX :: Position -> Int
getX (NewPosition x y) = x

-- getY returns the y of a position
getY :: Position -> Int
getY (NewPosition x y) = y
...
rgb (Mix color1 color2) =
  [ (c1 + c2) / 2 | (c1, c2) <- zip (rgb color1) (rgb color2) ]
rgb (Invert color) = [ 1 - c | c <- rgb color ]
