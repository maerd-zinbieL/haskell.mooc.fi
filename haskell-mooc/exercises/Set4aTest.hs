...
m_ex1_false example = forAll_ $ \(v, NonEmpty vs) ->
  not (elem (v `asTypeOf` example) vs)
    ==> $(testing [|allEqual (v:vs)|]) (?== False)
...
ex1_one = forAll_ $ \(Positive cnt, x :: Int, y) ->
  forAllBlind (shuffle (y : replicate cnt x))
    $ \xs -> x /= y ==> $(testing [|allEqual xs|]) (?== False)

m_ex2 domain = forAllBlind (sublistOf domain) $ \xs -> conjoin
  [ forAllBlind (shuffle xs) $ \xs' -> $(testing [|distinct xs'|]) (?== True)
  , not (null xs) ==> forAllBlind (shuffle (head xs : xs)) $ \xs' ->
    $(testing [|distinct xs'|]) (?== False)
  ]
...
m_ex3 c =
  let d = succ c
      e = succ d
  in  forAllBlind (shuffle [c, d, e])
        $ \[x, y, z] -> $(testing [|middle x y z|]) (?== d)
...
m_ex4 delta =
  forAllBlind (genRangeOf delta) $ \vs -> $(testing [|rangeOf vs|]) (?== delta)
...
ex5_int = forAllBlind (choose (2, 10)) $ \l ->
  forAllBlind (vectorOf l (arbitrary :: Gen Int)) $ \ans ->
    forAllBlind (listOf (choose (1, l - 1) >>= vector)) $ \rest ->
      forAllBlind (shuffle (ans : rest))
        $ \input -> $(testing [|longest input|]) (?== ans)
...
ex7 = conjoin
  [ forAll_ $ \(Small i) ->
    $(testing [|average [fromIntegral i :: Float]|]) (?== fromIntegral i)
  , forAllBlind (choose (1, 10)) $ \n -> forAll_
    $ \(i :: Rational) -> $(testing [|average (replicate n i)|]) (?== i)
  , forAll_ $ \(Small i) ->
    let f = fromIntegral i in $(testing [|average [f-1,f,f+1]|]) (?== f)
  , forAll_ $ \(Small i, Small j) ->
    let x = fromIntegral i :: Double
        y = fromIntegral j :: Double
    in  $(testing [|average (replicate 10 x ++ replicate 10 y)|])
          (?~= (x + y) / 2)
  ]

ex8_winner = property $ do
  names <- shuffle ["Mike", "Bob", "Lisa", "Jane"]
  let (win : lose : third : _) = names
  loseScore  <- choose (0, 10000)
  diff       <- choose (1, 100)
  thirdScore <- choose (0, 20000)
  let inp = Map.fromList
        [(win, loseScore + diff), (lose, loseScore), (third, thirdScore)]
  return $ conjoin
...
ex9_bool = forAllBlind (choose (0, 10)) $ \n ->
  forAllBlind (vectorOf n arbitrary) $ \bs ->
    $(testing [|freqs bs|]) . was $ \m ->
      ...
      in  conjoin
            [ counterexample
              ("  Number of True values should be " ++ show ntrue)
            $      null t
            ||     (True, ntrue)
            `elem` out
            , counterexample
              ("  Number of False values should be " ++ show nfalse)
            $      null f
            ||     (False, nfalse)
            `elem` out
            ]
...
ex9_int = forAllBlind (choose (0, 15)) $ \n ->
  forAllBlind (vectorOf n arbitrary) $ \(is :: [Int]) ->
    $(testing [|freqs is|]) . was $ \out ->
      let vals = nub is
      in  counterexample "  Number of keys"
                         (length (Map.keys out) ?== length vals)
            .&&. conjoin (map (ck out is) vals)
 where
  ck out vals i =
    let exp = length (filter (== i) vals)
    in  counterexample ("  Should contain " ++ show (i, exp))
          $      (i, exp)
          `elem` (Map.assocs out)

ex10_success = property $ do
  names <- shuffle ["Mike", "Bob", "Lisa", "Jane"]
  ...
  amount   <- choose (0, 100)
  let inp = Map.fromList
        [(from, fromBal + amount), (to, toBal), (third, thirdBal)]
  let out =
        Map.fromList [(from, fromBal), (to, toBal + amount), (third, thirdBal)]
  return $ $(testing [|transfer from to amount inp|]) (?== out)

ex10_missing = property $ do
  names <- shuffle ["Mike", "Bob", "Lisa", "Jane"]
...
ex11_swap_large = property $ do
  ...
  let out = listArray
        (base, max)
        (map (\x -> if x == vi then vj else if x == vj then vi else x) vals)
  return $ $(testing [|swap i j inp|]) (?== out)

ex12_bool = property $ do
  small <- choose (0, 100 :: Int)
  diff  <- choose (1, 100)
  return $ conjoin
    [ $(testing [|maxIndex (array (False,True) [(False,small),(True,small+diff)])|])
      (?== True)
    , $(testing [|maxIndex (array (False,True) [(True,small),(False,small+diff)])|])
      (?== False)
    ]

ex12_int = property $ do
  ...
  let inp = array
        (low, hi)
        [ (i, if i == mid then val + 1 else val) | i <- [low .. hi] ]
