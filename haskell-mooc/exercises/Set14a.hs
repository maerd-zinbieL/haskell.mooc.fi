...
greetText name | T.length name <= 15 = T.pack "Hello, " <> name <> T.pack "!"
...
shout text = T.unwords $ go $ T.words text
 where
  go :: [T.Text] -> [T.Text]
  go (w0 : w1 : rest) = T.toUpper w0 : w1 : go rest
  go [w             ] = [T.toUpper w]
  go []               = []
...
longestRepeat txt =
  foldl (\folded x -> max folded (T.length x)) 0 (T.group txt)
...
takeStrict n ltxt = TL.toStrict $ TL.take n ltxt
