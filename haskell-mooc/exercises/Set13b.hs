...
visit maze place = do
  ...
  if place `elem` visited
    then return ()
      ...
      put (place : visited)
      mapM_ (visit maze) (neibours place maze)
 where
  neibours place ((entry, entries) : rest) | place == entry = entries
                                           | otherwise = neibours place rest
  neibours place [] = []
...
allSums :: [Int] -> [Int]
allSums []       = [0]
allSums (x : xs) = [ x + s | s <- sums ] ++ sums where sums = allSums xs
...
sumBounded bound xs = foldM (f1 bound) 0 xs

f1 :: Int -> Int -> Int -> Maybe Int
f1 bound acc x | (acc + x) > bound = Nothing
...
instance Functor SL where
  ...
  -- f'' :: Int -> (b, Int, [String])
  fmap f (SL f') = SL f''
    where f'' i = let (x, state, log) = f' i in (f x, state, log)
...
instance Monad SL where
  ...
  SL fa >>= cont = SL fb
   where
    fb state0 =
      let (valA, state1, log1) = fa state0
          (SL fb')             = cont valA
          (valB, state2, log2) = fb' state1
      in  (valB, state2, log1 ++ log2)
...
--  *Set11b> get
--  4
getter :: IORef Int -> IO Int
getter ref = do val <- readIORef ref
                return val
