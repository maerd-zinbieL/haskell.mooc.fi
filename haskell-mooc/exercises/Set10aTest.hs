...
ex1_finite = forAllBlind (choose (0, 10)) $ \n ->
  forAllBlind (choose ('a', 'z')) $ \c ->
    forAllBlind (choose ('a', 'z')) $ \d ->
      $(testing [|doublify (replicate n c ++ [d])|])
        (?== (replicate (2 * n) c ++ [d, d]))
...
ex2_finite = forAll_ $ \(is :: [Integer]) -> forAll_ $ \(rest :: [Integer]) ->
  let as = odds is
      bs = evens is
  in  conjoin
...
ex2_infinite_1 = forAll_ $ \(i :: Integer, j :: Integer) ->
  counterexample ("with i = " ++ show i ++ ", j = " ++ show j)
    $ $(testing' [|take 10 (interleave (repeat i) (repeat j))|])
        (?== take 10 (cycle [i, j]))
...
ex3_examples = conjoin
  [ $(testing [|deal ["Hercule","Ariadne"] ["Ace","Joker","Heart"]|])
...
ex3_finite = forAllShrink_ (listOf1 word) $ \names ->
  forAllShrink_ (listOf1 word) $ \cards ->
    and [not (null names), not (null cards)] ==> $(testing [|deal names cards|])
      (?== [ (cards !! i, names !! (mod i (length names)))
           | i <- [0 .. length cards - 1]
           ]
      )
  ...
  not (null names)
    ...
    $   \n ->
          counterexample ("With n = " ++ show n ++ ", names = " ++ show names)
            $ $(testing' [|take n (deal names (map show [0..]))|])
                (?== [ (show i, names !! (mod i (length names)))
                     | i <- [0 .. n - 1]
                     ]
                )

ex3_infinite_2 = $(testing' [|take 10 (deal (repeat "me") (repeat "card"))|])
  (?== replicate 10 ("card", "me"))



ex4_simple = conjoin
  [ $(testing [|averages [1.0,2.0,3.0]|])
    (approximateListEq [1.0, (1 + 2) / 2, (1 + 2 + 3) / 3])
  , $(testing [|averages [7,2,5,8]|])
    (approximateListEq [7, (7 + 2) / 2, (7 + 2 + 5) / 3, (7 + 2 + 5 + 8) / 4])
  ...
  forAllBlind (elements [1, 2, 3, 4, 5, 6]) $ \step ->
    forAllBlind (choose (2, 7)) $ \len ->
      let input = [ base + i * step | i <- [0 .. len] ]
          output =
            [ (j * base + (j - 1) * j * step / 2) / j | j <- [1 .. len + 1] ]
...
ex5_examples = conjoin
  [ $(testing' [|take 20 (alternate "abc" "def" ',')|])
    (?== "abc,def,abc,def,abc,")
  , $(testing' [|take 10 (alternate [1,2] [3,4,5] 0)|])
    (?== [1, 2, 0, 3, 4, 5, 0, 1, 2, 0])
  ]
...
ex6_finite = forAll_ $ \(is :: [Int]) -> forAll_ $ \(NonNegative n) ->
...
ex8_works = $(hasType' "ignorecase" "String -> IgnoreCase") $ \ignorecase ->
  $(withInstance "Eq" "IgnoreCase" [|(==)|]) $ \(==) -> property $ do
    w1 <- word
    w2 <- shuffleCase w1
    w3 <- word
    let
      t :: String -> String -> Bool -> Property
      t x y exp =
        counterexample ("ignorecase " ++ show x ++ " == ignorecase " ++ show y)
...
ex9_1 = forAllShrink_ (choose (0, 20)) $ \i ->
  counterexample ("with i = " ++ show i) $ conjoin
    [ $(testing' [|play maze (replicate i "Left")|])
      (?== take
        (i + 1)
        (cycle ["Maze", "Deeper in the maze", "Elsewhere in the maze"])
      )
    , $(testing' [|play maze (replicate i "Right")|])
      (?== take
        (i + 1)
        (cycle ["Maze", "Elsewhere in the maze", "Deeper in the maze"])
      )
    ]

ex9_2 =
  forAllShrinkBlind (listOf (elements ["Left", "Right"])) subterms $ \dirs ->
    let cnt = sum
          (map
            (\d -> case d of
              "Left"  -> 1
        ...
        answer =
          ["Maze", "Deeper in the maze", "Elsewhere in the maze"] !! mod cnt 3
