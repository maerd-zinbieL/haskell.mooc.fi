...
split s | ' ' `elem` s = Just (fst, snd)
        | otherwise    = Nothing
  where fst : snd : _ = words s
...
checkNumber (s1, s2) | any isDigit s1 || any isDigit s2 = Nothing
...
winner scores player1 player2 = do
  ...
  if (s1 >= s2) then return player1 else return player2
-- winner scores player1 player2 = score player1 player2 ?> compare
--   where score player1 player2 = case (lookup player1 scores, lookup player2 scores) of
--                                      (Just s1, Just s2) -> Just (s1, s2)
--                                      (_, _) -> Nothing
--         compare (s1, s2) | s1 >= s2 = Just player1
...
-- selectSum _  []       = Just 0
-- selectSum xs (i : is) = selectSum xs is >>= add (safeIndex xs i)
--  where
--   add Nothing  _   = Nothing
--   add (Just x) sum = Just (x + sum)
...
instance Monad Logger where
  return x = Logger [] x
  Logger la a >>= f = Logger (la ++ lb) b where Logger lb b = f a
...
countAndLog pre [] = Logger [] 0
countAndLog pre (x : xs)
  | pre x = do
    count <- countAndLog pre xs
    msg (show x)
    return (count + 1)
  | otherwise = countAndLog pre xs

-- countAndLog pre [] = Logger [] 0
-- countAndLog pre (x : xs) | pre x = countAndLog pre xs >>= continue [(show x)]
--                          | otherwise = countAndLog pre xs
--  where
--   continue :: [String] -> Int -> Logger Int
--   continue x count = Logger x (count + 1)
...
exampleBank :: Bank
exampleBank =
  (Bank (Map.fromList [("harry", 10), ("cedric", 7), ("ginny", 1)]))

balance :: String -> BankOp Int
balance accountName = BankOp getBalance
 where
  getBalance :: Bank -> (Int, Bank)
  getBalance p@(Bank accounts) =
    (Map.findWithDefault 0 accountName accounts, p)
------------------------------------------------------------------------------
...
parensMatch s = count == 0 where (_, count) = runState (mapM_ paren s) 0
...
count x = state (go x)
 where
  ...
  go x ((x', i) : ps)
    | x' == x   = ((), (x, i + 1) : ps)
    | otherwise = let ((), ps') = go x ps in ((), (x', i) : ps')
...
occurrences xs = state (go xs)
 where
  go :: (Eq a) => [a] -> [(a, Int)] -> (Int, [(a, Int)])
  go [] states = (length states, states)
  go (x : xs) states =
    let (_, states' ) = go xs states
        (_, states'') = runState (count x) states'
    in  (length states'', states'')
