...
firstHalf str | even len  = take (div len 2) str
              | otherwise = take (div len 2 + 1) str
...
powers :: Int -> Int -> [Int]
powers k max = takeWhile (<= max) [ k ^ p | p <- [0 .. max] ]
...
whileRight f x = next (f x)
 where
  next (Left  contents) = contents
  next (Right contents) = whileRight f contents
...
joinToLength len strs =
  filter (\str -> length str == len) [ x ++ y | x <- strs, y <- strs ]
...
sumRights xs = sum (map (either left right) xs)
 where
  left _str = 0
  right int = int
...
interpreter' []              _ _ = []
interpreter' (_ : cs)        x y = "Unsupport" : interpreter' cs x y
