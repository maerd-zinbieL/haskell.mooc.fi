    ...
    forAllBlind (vectorOf (sum ns) letter) $ \inp ->
      ...
      in
        conjoin
          ...
          , not (null inp) ==> forAllBlind
            (choose (1, length inp))
            (\k -> $(testing [|sumIsLength (drop k inp) out|]) qFail)
          , forAllBlind
            (choose (1, length out))
            (\k ->
              sum (take k ns)
                /=  0
                ==> $(testing [|sumIsLength inp (drop k out)|]) qFail
            )
          ]
  ...
  forAllBlind (vectorOf (length inp) (choose (0, 100))) $ \ns ->
    forAllBlind (shuffle (zip (nub inp) ns)) $ \out ->
      forAllBlind (sublistOf inp `suchThat` (not . null)) $ \inp' ->
        forAllBlind (sublistOf out) $ \out' -> conjoin
...
changeCount (p : ps) = oneof [fmap (p :) $ changeCount ps, fmap (: ps) $ chg p]
      ...
      forAllBlind (shuffle (zip (nub cs) ns)) $ \out ->
        forAllBlind (shuffle (outToIn out)) $ \inp ->
          forAllBlind (sublistOf out `suchThat` (not . null)) $ \out' ->
            forAllBlind (sublistOf inp) $ \inp' -> conjoin
              [ $(testing [|outputInInput inp out'|]) qPass
              , inp' /= inp ==> $(testing [|outputInInput inp' out|]) qFail
              , forAllBlind (changeCount out)
                $ \out2 -> $(testing [|outputInInput inp out2|]) qFail
              ]
...
ex6_1 = testGen genList $ \l -> conjoin
  [ counterexample "  length should be 3-5" $ length l >= 3 && length l <= 5
  , counterexample "  all elements should be 0-10"
    $ all (\x -> x >= 0 && x <= 10) l
  , counterexample "  list should be sorted" $ sort l == l
  ]

ex6_2 = once $ testGenMany genList 30 $ \ls -> conjoin
  [ counterexample "  should have lists of all lengths"
  $   nub (sort (map length ls))
  ?== [3, 4, 5]
  , counterexample "  should at least 7 different ints"
  $  length (nub (sort (concat ls)))
  >= 7
  , counterexample "  should have a list with differing elements"
    $ any ((> 1) . length . nub) ls
  ]

ex7_arg_1 =
  counterexample "arbitrary :: Gen Arg"
    $ testGen (arbitrary :: Gen Arg)
    $ \a -> case a of
        Number i ->
          counterexample "  number should be in the range 0-10"
            $  i
            >= 0
            && i
            <= 10
        Variable c ->
          counterexample "  variable should be one of abcxyz" $ elem c "abcxyz"

ex7_arg_2 =
  once
    $ counterexample "arbitrary :: Gen Arg"
    $ testGenMany (arbitrary :: Gen Arg) 20
    $ \as ->
        counterexample "  should generate both Number and Variable values"
          $  any isVariable         as
          && any (not . isVariable) as
 where
...
ex7_exp =
  once
    ...
    $ testGenMany (arbitrary :: Gen Expression) 64
    $ \es ->
