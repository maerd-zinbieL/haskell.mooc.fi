...
statements ss0 ss1 = liftA2 is ss0 ss1 ++ liftA2 is ss0 (not <$> ss1)
 where
  is s0 s1 = s0 ++ " is " ++ s1
  not s = "not " ++ s
------------------------------------------------------------------------------
...
calculator opStr valStr = liftA2 runOp parsedOp parsedVal
 where
  runOp op value = op value
  parsedOp | opStr == "negate" = Just (0 -)
           | opStr == "double" = Just (* 2)
           | otherwise         = Nothing
  parsedVal = readMaybe valStr
...
validateDiv p q = check (q /= 0) "Division by zero!" (p `div` q)
...
validateAddress streetName streetNumber postCode = liftA2
  (\completeAddr code -> completeAddr code)
  (liftA2 Address validatedName validatedNumber)
  validatedCode
 where
  validatedName =
  ...
  validatedCode = check (all isDigit postCode && (length postCode == 5))
                        "Invalid postcode"
                        postCode
          ------------------------------------------------------------------------------
...
liftPerson fName fAge fEmployed = liftA2
  (\completePerson employed -> completePerson employed)
  (liftA2 Person fName fAge)
  fEmployed
...
twoPersons
  ...
  -> f Bool
  -> f [Person]
twoPersons name1 age1 employed1 name2 age2 employed2 = liftA2
  (\p1 p2 -> [p1, p2])
  (liftPerson name1 age1 employed1)
  (liftPerson name2 age2 employed2)
...
validateBool s | s == "True"  = pure (Left True)
...
normalizePhone str = liftA2 const validatedLen validatedDigit
 where
  strip = concat . words $ str
  checkNumber x = check (isDigit x) ("Invalid character: " ++ [x]) x
  validatedDigit = traverse checkNumber strip
  validatedLen   = check (length strip <= 10) "Too long" strip
------------------------------------------------------------------------------
...
parseOp op | op == "+" = pure Plus
           | op == "-" = pure Minus
           | otherwise = invalid ("Unknown operator: " ++ op)

parseArgNum arg =
  check (all isDigit arg) ("Invalid number: " ++ arg) (Number (read arg :: Int))
parseArgVar arg = check (length arg == 1 && isAlpha (head arg))
                        ("Invalid variable: " ++ arg)
                        (Variable (head arg))

parseExpression' str = liftA2
  (\op arg2 -> op arg2)
  (liftA2 (\op arg1 -> op arg1) parsedOp parsedArg1)
  parsedArg2
 where
  [arg1, op, arg2] = words str
  parsedOp         = parseOp op
  parsedArg1       = parseArgNum arg1 <|> parseArgVar arg1
  parsedArg2       = parseArgNum arg2 <|> parseArgVar arg2

parseExpression :: String -> Validation Expression
parseExpression str | length (words str) == 3 = parseExpression' str
...
instance Applicative Priced where
  pure x = Priced 0 x
  liftA2 f (Priced v1 x1) (Priced v2 x2) = Priced (v1 + v2) (f x1 x2)
...
instance (Functor f, Functor g) => Functor (Both f g) where
  ...
  fmap func (Both both) = Both (fmap (fmap func) both)
...
-- I cheated : https://stackoverflow.com/questions/58054217/composing-applicatives
instance (Applicative f, Applicative g) => Applicative (Both f g) where
  pure x = Both (pure (pure x))
  liftA2 abc (Both fga) (Both fgb) = Both (liftA2 (liftA2 abc) fga fgb)
