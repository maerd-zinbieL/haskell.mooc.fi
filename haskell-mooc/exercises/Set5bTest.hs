...
ex1_valAtRoot_Nothing =
  $(testing [|valAtRoot (Empty :: Tree Bool)|]) (?== (Nothing :: Maybe Bool))
...
ex2_treeSize = forAllShrink_ (choose (0, 50)) $ \s ->
...
m_ex3 depth = forAllBlind (treeAndMax depth)
  $ \(m, tree) -> $(testing [|treeMax tree|]) (?== m)

ex3_small = conjoin [m_ex3 0, m_ex3 1, m_ex3 2]
ex3_large = conjoin [m_ex3 3, m_ex3 4]
...
ex4_small = property $ do
  ...
  let input = tree3 a b c
      out   = all (> 0) [a, b, c]
  return
    $   counterexample ("allValues (>0) " ++ show' input)
    $   allValues (> 0) input
    ?== out
...
ex5_small = property $ do
  ...
  return
    $   counterexample ("mapTree (+1) " ++ show' input)
    $   mapTree (+ 1) input
    ?== output
...
replaceLeaf t (Node val l r) = oneof
...
orderedTree depth (min, max) | min < max = frequency
  [ (1, return Empty)
  , ( 4
    , do
      v <- choose (min, max - 1)
...
m_ex7 depth = forAllBlind (orderedTree depth (0, 100)) $ \t ->
  forAllBlind (breakOrder t) $ \b -> conjoin
...
ex7_small = conjoin
  ...
  , forAllBlind (choose (0, 10 :: Int))
    $ \i -> $(testing [|isOrdered (Node i Empty Empty)|]) (?== True)
  , m_ex7 2
  ]

ex7_large = conjoin [m_ex7 3, m_ex7 4, m_ex7 5]


pathAndTree :: (Eq a, Arbitrary a) => Int -> a -> Gen ([Step], Tree a)
pathAndTree 0     v = return ([], Node v Empty Empty)
pathAndTree depth v = oneof
  [ do
    l      <- treeWithout (depth - 1) v
    v'     <- arbitrary `suchThat` (/= v)
...
m_ex8 d = forAll_ $ \(v :: Int) -> forAllBlind (pathAndTree d v)
  $ \(p, t) -> $(testing [|walk p t|]) (?== Just v)

ex8_small = conjoin
  [ $(testing [|walk [StepL] (Empty :: Tree Char)|]) (?== Nothing)
  , forAll_ $ \(a :: Int, b :: Int, c :: Int) ->
    $(testing [|walk [StepL,StepR] (Node a (Node b Empty Empty) (Node c Empty Empty))|])
      (?== Nothing)
  , m_ex8 0
  , m_ex8 1
  , m_ex8 2
...
ex8_large = conjoin [m_ex8 3, m_ex8 4, m_ex8 5]


ex9_small = conjoin
  [ $(testing [|set ([]::[Step]) 'a' (Empty::Tree Char)|]) (?== Empty)
  , $(testing [|set ([]::[Step]) True (Node False Empty Empty)|])
    (?== Node True Empty Empty)
  , $(testing [|set [StepL] True (Node False Empty Empty)|])
    (?== Node False Empty Empty)
  , $(testing [|set [StepL] True (Node False (Node False Empty Empty) Empty)|])
    (?== Node False (Node True Empty Empty) Empty)
  ]

setList xs i v = before ++ v : after where (before, _ : after) = splitAt i xs
...
ex9_large = forAllShrink (choose (1, 10)) shrinkPositive $ \len ->
  property $ do
    (dir, mk) <- elements [(StepL, linearLeft), (StepR, linearRight)]
    cs        <- vectorOf len (choose ('a', 'z'))
    i         <- choose (0, len - 2)
    new       <- choose ('a', 'z')
    return $ $(testing [|set (replicate i dir) new (mk cs)|])
      (?== mk (setList cs i new))

m_ex10_yes depth = forAll_ $ \(val :: Int) ->
  forAllBlind (pathAndTree depth val)
    $ \(path, tree) -> $(testing [|search val tree|]) (?== Just path)

m_ex10_no depth = forAll_ $ \(val :: Int) ->
  forAllBlind (treeWithout depth val)
    $ \tree -> $(testing [|search val tree|]) (?== Nothing)
