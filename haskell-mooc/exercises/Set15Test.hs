...
ex1 = forAll_ $ \i -> forAll_ $ \j -> conjoin
...
ex2_1 = conjoin
  ...
  , $(testing [|statements ["x"] ["y","w"]|])
    (hasElements ["x is y", "x is w", "x is not y", "x is not w"])
...
ex2_2 = forAllBlind (resize 8 $ listOf1 word) $ \xs ->
  forAllBlind (resize 8 $ listOf1 word) $ \ys ->
    forAllBlind (elements xs) $ \x -> forAllBlind (elements ys) $ \y ->
      forAllBlind (elements [" is ", " is not "]) $ \i ->
        let exp = x ++ i ++ y
        in  $(testing [|statements xs ys|]) . was $ \out ->
              counterexample ("  Should contain " ++ show exp) $ elem exp out
...
ex3_failure_1 = forAllBlind word $ \w ->
  forAllBlind (choose (-100, 100 :: Int)) $ \i ->
    not (elem w ["negate", "double"])
      ==> $(testing [|calculator w (show i)|]) (?== Nothing)
...
ex3_failure_2 = forAll (elements ["negate", "double", "xyzzy"]) $ \w ->
  forAllBlind (choose (-100, 100 :: Int)) $ \i ->
    ...
    in  conjoin
          ...
          , forAllBlind (shuffle (s ++ "z"))
            $ \s' -> $(testing [|calculator w s'|]) (?== Nothing)
          ]
...
ex4 = forAllBlind (choose (0, 100)) $ \i ->
  forAllBlind (choose (1, 10)) $ \j -> conjoin
...
ex5_ok = forAllBlind postCode $ \p -> forAllBlind street $ \s ->
  forAllBlind (fmap show (choose (1, 1000 :: Int)))
    $ \n -> $(testing [|validateAddress s n p|]) (?== ok (Address s n p))

ex5_fail = forAllBlind postCode $ \p -> forAllBlind street $ \s ->
  forAllBlind (fmap show (choose (1, 1000 :: Int))) $ \n ->
      ...
      forAllBlind (vectorOf 20 (choose ('a', 'z'))) $ \pad ->
        forAllBlind (shuffle (chaff : take 4 p)) $ \p' ->
          forAllBlind (shuffle (chaff : n)) $ \n' -> conjoin
            [ $(testing [|validateAddress (s++pad) n p|])
              (?== invalid "Invalid street name")
            , $(testing [|validateAddress s n (take 4 p)|])
              (?== invalid "Invalid postcode")
            , $(testing [|validateAddress s n p'|])
              (?== invalid "Invalid postcode")
            , $(testing [|validateAddress s n' p|])
              (?== invalid "Invalid street number")
            , $(testing [|validateAddress (s++pad) n p'|])
              (?== invalids ["Invalid street name", "Invalid postcode"])
            , $(testing [|validateAddress (s++pad) n' (take 4 p)|])
              (?== invalids
                [ "Invalid street name"
...
ex6_maybe = forAllBlind word $ \name1 -> forAllBlind word $ \name2 ->
  forAllBlind (choose (10, 100)) $ \age1 ->
    forAllBlind (choose (10, 100)) $ \age2 -> forAll_ $ \(emp1, emp2) -> conjoin
...
ex6_list = forAllBlind word $ \name1 -> forAllBlind word $ \name2 ->
  forAllBlind word $ \name3 -> forAllBlind (choose (10, 100)) $ \age1 ->
    forAllBlind (choose (10, 100)) $ \age2 ->
      forAllBlind (choose (10, 100)) $ \age3 -> forAll_ $ \(emp1, emp2) ->
        conjoin
...
ex7_ok = forAll_ $ \(i :: Int) -> forAll_ $ \(b :: Bool) -> conjoin
...
sow [] ys = return ys
sow (x : xs) (y : ys) =
  oneof [fmap (x :) $ sow xs (y : ys), fmap (y :) $ sow (x : xs) ys]

ex7_fail = forAll_ $ \(i :: Int) -> forAllBlind (choose ('a', 'n')) $ \c -> -- don't generate 0o or 0x by accident
...
ex8 = forAllBlind (take 10 <$> listOf1 digit) $ \out ->
  forAllBlind (listOf1 (return ' ')) $ \spaces -> forAllBlind word $ \chaff ->
    forAllBlind (sow out spaces) $ \inp ->
      forAllBlind (sow inp chaff) $ \wrong -> conjoin
        [ $(testing [|normalizePhone inp|]) (?== ok out)
        , $(testing [|normalizePhone (inp++"9999999999")|])
          (?== invalid "Too long")
        , $(testing [|normalizePhone wrong|])
          (?== invalids
            (  (if length out + length chaff > 10 then ["Too long"] else [])
            ++ [ "Invalid character: " ++ [d] | d <- chaff ]
            )
          )
        ]
...
genA = oneof [Number <$> choose (0, 10), Variable <$> choose ('a', 'z')]

genE = elements [Plus, Minus] <*> genA <*> genA

ex9_ok = forAllBlind genE $ \exp ->
  let s = showE exp in $(testing [|parseExpression s|]) (?== ok exp)
      ...
      forAllBlind (elements ["*", "%", "x", "3"]) $ \badOp ->
        ...
        in  conjoin
              [ $(testing [|parseExpression (b goodArg goodOp badArg)|])
                (?== invalids
                  ["Invalid number: " ++ badArg, "Invalid variable: " ++ badArg]
                )
              ...
              , $(testing [|parseExpression (b goodArg badOp badArg)|])
                (?== invalids
                  ...
                  , "Invalid number: " ++ badArg
                  , "Invalid variable: " ++ badArg
                  ]
                )
              , $(testing [|parseExpression (b goodArg badOp badArg)|])
                (?== invalids
...
ex10_liftA2 = conjoin
  [ $(testing' [|liftA2 (++) (Priced 3 "abc") (Priced 5 "de")|])
    (?== Priced 8 "abcde")
...
ex11 = conjoin
  ...
  , $(testing' [|[reverse,drop 1] <#> ["foo","bar"]|])
    (?== ["oof", "rab", "oo", "ar"])
  ]
...
ex13_validation = forAllBlind (choose (0 :: Int, 10)) $ \l ->
  forAllBlind (listOf1 (choose (0 :: Int, 10))) $ \xs -> tA
    l
    xs
    (?== case filter (> l) xs of
...
ex14_ok = conjoin
  ...
  , $(testing' [|fmap negate (Both [Just 3, Just 4])|])
    (?== Both [Just (-3), Just (-4)])
  , $(testing' [|fmap negate (Both [[1,2,3],[4,5]])|])
    (?== Both [[-1, -2, -3], [-4, -5]])
  ]
...
ex15_pure = conjoin
  ...
  , $(testing' [|pure "a" :: Both Maybe Validation String|])
    (?== Both (Just (ok "a")))
  ]

ex15_liftA2 = conjoin
  [ $(testing' [|liftA2 (+) (Both (Just [10,100])) (Both (Just [1,2]))|])
    (?== Both (Just [11, 12, 101, 102]))
  , $(testing' [|liftA2 (+) (Both (Just [10,100])) (Both Nothing)|])
    (?== Both Nothing)
  ]

ex15_liftA2_2 = conjoin
  [ $(testing' [|liftA2 (&&) (Both (Just (invalid "err"))) (Both (Just (pure True)))|])
    (?== Both (Just (invalid "err")))
  , $(testing' [|liftA2 (&&) (Both (Just (invalid "err"))) (Both (Just (invalid "umm")))|])
    (?== Both (Just (invalids ["err", "umm"])))
