...
tests =
  ...
  , ( 11
    , "addEithers"
    , [ex11_addEithers_both, ex11_addEithers_one, ex11_addEithers_neither]
    )
  ]
...
ex1_years =
  ...
  in  let length = origLength :: [a] -> Int
      in  conjoin
            [ $(testing' [|length years|]) (?== 3)
...
ex3_updateAt = property $ do
  ...
  w  <- word
  return . $(testing [|updateAt i w ws|]) $ \v -> conjoin
    [ counterexample ("index " ++ show i) $ v !! i ?== w
    , counterexample ("index " ++ show j) $ i /= j ==> v !! j ?== ws !! j
    ]

ex4_substring = forAll_ $ \(prefix :: String, target, suffix) ->
  $(testing [|substring (length prefix) (length prefix + length target) (prefix ++ target ++ suffix)|])
    (?== target)

ex5_isPalindrome = forAllBlind word $ \w1 ->
  forAllBlind
      (word `suchThat` ((/= head w1) . last) `suchThat` ((> 1) . length))
    $ \w2 -> conjoin
        [ $(testing [|isPalindrome (w1 ++ w2)|]) (?== False)
...
ex7_safeDiv_zero = forAll_ $ \x -> $(testing [|safeDiv x 0|]) (?== Nothing)

ex7_safeDiv_nonZero = forAll_
  $ \(x, y) -> (y /= 0) ==> $(testing [|safeDiv x y|]) (?== Just (div x y))

ex8_greet_nothing = forAllBlind word $ \first ->
  $(testing [|greet first (Nothing :: Maybe String)|])
    (?== ("Hello, " ++ first ++ "!"))

ex8_greet_just = forAllBlind word $ \first -> forAllBlind word $ \last ->
  $(testing [|greet first (Just last)|])
    (?== ("Hello, " ++ first ++ " " ++ last ++ "!"))
...
ex9_safeIndex_fail = property $ do
  ...
  return $ conjoin
    [ $(testing [|safeIndex is neg|]) (?== Nothing)
    , $(testing [|safeIndex is over|]) (?== Nothing)
    ]

ex10_eitherDiv_zero =
  forAll_ $ \x -> $(testing [|eitherDiv x 0|]) (?== Left (show x ++ "/0"))

ex10_eitherDiv_nonZero = forAll_
  $ \(x, y) -> (y /= 0) ==> $(testing [|eitherDiv x y|]) (?== Right (div x y))


ex11_addEithers_both = forAll_ $ \(x, y) ->
  $(testing [|addEithers (Right x :: Either String Int) (Right y :: Either String Int)|])
    (?== Right (x + y))
...
ex11_addEithers_neither = forAll_ $ \(a, b) ->
  $(testing [|addEithers (Left a :: Either String Int) (Left b :: Either String Int)|])
    (?== Left a)
