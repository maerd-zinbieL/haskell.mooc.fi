...
allEqual []             = True
allEqual [x           ] = True
allEqual (x0 : x1 : xs) = x0 == x1 && allEqual (x1 : xs)
...
middle a b c | a >= max b c = max b c
...
rangeOf []       = 0
rangeOf (x : xs) = rangeOfHelper xs x x

rangeOfHelper :: (Num a, Ord a) => [a] -> a -> a -> a
rangeOfHelper [] maxValue minValue = maxValue - minValue
rangeOfHelper (x : xs) maxValue minValue =
  rangeOfHelper xs (max maxValue x) (min minValue x)
...
longest :: Ord a => [[a]] -> [a]
longest = foldr compareList []

compareList :: Ord a => [a] -> [a] -> [a]
compareList l1 l2
  | length l1 == length l2 = if head l1 <= head l2 then l1 else l2
...
incrementKey k [] = []
incrementKey k ((k', v') : kvs) | k == k'   = (k', v' + 1) : incrementKey k kvs
...
transfer from to amount bank
  | canTransfer from to amount bank = transfer' (-)
                                                amount
                                                from
...
transfer'
  ...
  -> Map.Map String Int
  -> Map.Map String Int
transfer' op amount = Map.alter update
 where
  update v = case v of
    Just v' -> Just (op v' amount)
    Nothing -> Nothing

canTransfer :: String -> String -> Int -> Map.Map String Int -> Bool
canTransfer from to amount bank
...
maxIndex arr = maxIndexHelper ls max
 where
  ls  = Data.Array.assocs arr
  max = head ls

maxIndexHelper :: (Ix i, Ord a) => [(i, a)] -> (i, a) -> i
maxIndexHelper [] (iMax, vMax) = iMax
maxIndexHelper ((i, v) : ivs) (iMax, vMax)
  | v > vMax  = maxIndexHelper ivs (i, v)
  | otherwise = maxIndexHelper ivs (iMax, vMax)
